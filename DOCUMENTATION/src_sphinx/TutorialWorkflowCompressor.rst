...
#. the script :download:`prepare.py <../../EXAMPLES/WORKFLOW_COMPRESSOR/rotor37_SingleCase/prepare.py>`
...
:download:`prepare.py <../../EXAMPLES/WORKFLOW_COMPRESSOR/rotor37_SingleCase/prepare.py>`.
...
#. ``splitOptions=dict(mode='auto')``: automatically search for the optimum
   ...
   for Cascade Lake nodes on **sator**). The number of nodes to use is an output (so
...
* ``setup.py``: an ultra-light file containing all relevant information of the
  simulation. It must be consistent with ``main.cgns``. We can find all the
...
.. note::
    ...
    recommended, but could be useful for developers for example), you can
...
.. note::
    **Submission on a distant machine**: If you want to copy all the templates
    ...
    In that case, just after the end of execution of your script `prepare.py`,
...
* the :py:class:`dict` **CoprocessOptions** in the :py:class:`dict`
  **ReferenceValues** (firstly defined in ``prepare.py``, then written
  in ``setup.py``)

* the :py:class:`list` **Extractions** (also firstly defined in
  ``prepare.py``, then written in ``setup.py``)
...
The file ``arrays.cgns`` may contain different kinds of data:

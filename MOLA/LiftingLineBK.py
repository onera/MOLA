...
def buildBodyForceDisk(Propeller, PolarsInterpolatorsDict, NPtsAzimut,
    ...
    RotAxis = I.getValue(I.getNodeFromName1(Kin_n,'RotAxis'))
    RotCenter = I.getValue(I.getNodeFromName1(Kin_n,'RotCenter'))
    RightHandRuleRotation = I.getValue(I.getNodeFromName1(Kin_n,'direct'))
    ...
    fVxyz = I.getNodeFromName1(Cond_n,'FreestreamVelocity')[1]
...
def computeSourceTerms(zone, SourceTermScale=1.0):
    '''
    zone must contain variables at FlowSolution (nodes):
    'Density', 'MomentumX','MomentumY', 'MomentumZ', 'EnergyStagnationDensity'
    'VelocityTangential', 'fx', 'fy', 'fz', 'ft'
    '''
...
def migrateSourceTerms2MainPyTree(donor, receiver):
    '''
    donor must be I.getZones() compatible element
    receiver must be a fully distributed PyTree
    '''
    Cmpi.barrier()
...
def buildPropeller(LiftingLine, NBlades, RotCenter=[0,0,0],
                   RotAxis=[1,0,0], GuidePoint=[0,1,0], direct=True):
    '''
    Construct a propeller object.

    A propeller object is a CGNSBase_t object, in which several
    LiftingLine objects are contained.
    Special motion nodes may be contained in the base.

    INPUTS

    LiftingLine - (PyTree 1D Zone) A Lifting Line object, as
        generated from buildLiftingLine()

    NBlades - (integer >0) Number of blades. Will make copies
        of LiftingLine.

    RotCenter - (3 element 1D numpy) - (x,y,z) coordinates of
        the rotation center of the propeller.

    RotAxis - (3 element 1D numpy) - Vector indicating the
        rotation axis direction, which points towards the
        suction side of the blades. If not unitary, the function
        will force to it to be unit vector.
    ...
    direct - (boolean) - Indicates if the propeller rotates in
        the right-hand-rule direction (True) or in the left-hand-
        rule direction (False)

    OUTPUT

    Propeller - (PyTree Base) - CGNSBase_t object representing
        the propeller
    '''

    RotCenter = np.array(RotCenter,  order='F', dtype=np.float)
    RotAxis   = np.array(RotAxis,    order='F', dtype=np.float)
    ...
    for nb in range(NBlades):
        ...
        if not direct: C._initVars(LiftingLine,'{CoordinateY}=-{CoordinateY}')
...
def buildLiftingLine(Span, **kwargs):
    '''
    Make a PyTree-Line zone defining a Lifting-line. The
    construction of this element is the same as in function
    GenerativeShapeDesign.wing(), same logic is used here!
    ...
    BEWARE: The lifting line is set towards the +X spanwise
    direction, -Y sweepwise direction and +Z dihedralwise.
    '''
    ...
    for GeomParam in LLDict:
        if GeomParam not in SpecialTreatment:
            LLDict[GeomParam][:] = J.interpolate__(RelSpan,
                                            ...
                                            kwargs[GeomParam]['InterpolationLaw'],
                                            **kwargs[GeomParam])
    ...
    for InitVars in ['AoA', 'Mach', 'Reynolds',
                     'Cl', 'Cd','Cm']:
        C._initVars(LiftingLine,InitVars,0.)
...
def checkComponentKind(component,kind='LiftingLine'):
    '''
    Function to determine whether a component is of kind given
    by attribute <kind>.

    INPUTS
    Component (CGNS node) - Component whose kind verification is
        desired.
...
def buildPolarsInterpolatorDict(PyZonePolars, InterpFields=['Cl', 'Cd'], Nrequest=None):
    """
    Build a Python dictionary of interpolation functions
    of polars from a list of PyZonePolars. Each key is the
    name of the PyZonePolar (the airfoil's tag), and the
    value is the interpolation function, e.g:
        InterpDict['MyPolar'](AoA,
                              Mach,
                              Reynolds)
    """
    InterpDict = {}
    for polar in PyZonePolars:
        PolarInterpNode = I.getNodeFromName1(polar,'.Polar#Interp')
        if PolarInterpNode is None:
            mode = 'RectBivariateSpline'
        else:
...
def buildPolarsAnalyticalDict(AnalyticalPolarsDict):
    """
        InterpDict['MyPolar'](AoA,
                              Mach,
                              Reynolds)
    ...
    for polar in AnalyticalPolarsDict:
        ...
        def analyticalPolar(AoA,Mach,Reynolds):
            try: CLmin = AnalyticalPolarsDict[polar]['CLmin']
            except KeyError: CLmin = -1.0
            try: CLmax = AnalyticalPolarsDict[polar]['CLmax']
            except KeyError: CLmax = 1.5
            try: CL0   = AnalyticalPolarsDict[polar]['CL0']
            except KeyError: CL0   = 0.0
            try: CLa   = AnalyticalPolarsDict[polar]['CLa']
            except KeyError: CLa   = 2*np.pi
            try: CD0   = AnalyticalPolarsDict[polar]['CD0']
            except KeyError: CD0   = 0.011
            try: CD2u   = AnalyticalPolarsDict[polar]['CD2u']
            except KeyError: CD2u   = 0.004
            try: CD2l   = AnalyticalPolarsDict[polar]['CD2l']
            except KeyError: CD2l   = 0.013
            try: CLCD0   = AnalyticalPolarsDict[polar]['CLCD0']
            except KeyError: CLCD0   = 0.013
            try: REref = AnalyticalPolarsDict[polar]['REref']
            except KeyError: REref = 1.e6
            try: REexp = AnalyticalPolarsDict[polar]['REexp']
            except KeyError: REexp = 0.
...
def buildLiftingLineInterpolator(LiftingLine, InterpFields=['Cl', 'Cd']):
    '''
    Build a LiftingLine Interpolator object.
    '''
    ...
    X._setInterpData(RequestLine, [DataSurface], order=2,
                     nature=1, loc='nodes', storage='direct', hook=None, method='lagrangian',dim=2)


    LiftingLineInterpolator = DataSurface, RequestLine
    return LiftingLineInterpolator

def interpolatorFromPyZonePolar(PyZonePolar, interpOptions=None, InterpFields=['Cl', 'Cd']):
    '''
...
def extractorFromPyZonePolar(PyZonePolar, Nrequest, InterpFields=['Cl', 'Cd']):
    '''
    This function create the interpolation function of Polar
    data of an airfoil stored as a PyTree Zone.
    ...
    def interpolationFunction(AoA, Mach, Reynolds):
        ...
        X._setInterpData(PolarRequestLine, [PolarDataSurface], order=2, nature=1, loc='nodes', storage='direct', hook=[hook],  method='lagrangian',dim=2)
...
def RbfInterpFromPyZonePolar(PyZonePolar, InterpFields=['Cl', 'Cd']):
    '''
    This function create the interpolation function of Polar
    data of an airfoil stored as a PyTree Zone.
...
def _applyLiftingLineInterpolator(LiftingLine,LiftingLineInterpolator, PolarsInterpolatorDict, InterpFields=['Cl','Cd']):
    '''
...
def _applyPolarOnLiftingLine(LiftingLine,PolarsInterpolatorDict, InterpFields=['Cl', 'Cd']):
    """
...
def _findOptimumAngleOfAttackOnLiftLine(LiftingLine,PolarsInterpolatorDict, Aim='Cl', AimValue=0.5, AoASearchBounds=(-2,6),SpecificSections=None, ListOfEquations=[]):
    """
    Store in the AoA FlowSolution the optimum angle of
    attack based on a given "Aim", using the provided
    PolarsInterpolatorDict as well as the existing Reynolds
    and Mach number values contained in FlowSolution.
...
def pyZonePolar2AirfoilZone(pyzonename,PyZonePolars):
    '''
    Conveniently use ".Polar#FoilGeometry" coordinates of a
    PyZonePolar in order to build a 1D zone.
    '''
    ...
    AirfoilGeom = J.createZone(pyzonename,[Xcoord,Ycoord,Ycoord*0],['CoordinateX','CoordinateY','CoordinateZ'])

    return AirfoilGeom

def resetPitch(LiftingLine,ZeroPitchRelativeSpan=ZeroPitchRelativeSpan,
               modifyLiftingLine=True):
    '''
    Given an existing LiftingLine object, reset the pitch taking
    as reference the value in attribute ZeroPitchRelativeSpan,
    which modifies in-place LiftingLine object (update of Twist)
    applying a DeltaTwist value such that the resulting Twist
    yield 0 degrees at ZeroPitchRelativeSpan. The value of
    DeltaTwist is returned by the function.
    '''
...
def remapLiftingLine(LiftingLine,NewRelativeDistribution,InterpolationLaw='interp1d_linear'):
    '''
    ...
    NewRelativeDistribution (1D PyTree zone or numpy array) -
        the new desired spanwise discretization
    ...
    OldAbscissa, = J.getVars(LiftingLine,['s'])
...
def postLiftingLine2Surface(LiftingLine, PyZonePolars, Variables=[],
    ...
    '''
    Post-process a Lifting Line element using enhanced
    PyZonePolars data in order to build surface fields
    (like Cp, theta, fields...) from a BEMT solution
    (or alternatively from PUMA solution converted into a
    LiftingLine equivalent CGNS object).

    INPUTS:

    LiftingLine (PyTree Zone) - Result of a BEMT computation
        or PUMA results adapted to CGNS following the same
        convention.

    PyZonePolars (list of PyTree Zones) - Enhanced PyZonePolars
        for each airfoil, containing also foilwise distributions
        fields (Cp, theta, delta*...)
    ...
    ChordDict = dict(RelativeSpan=s,Chord=Chord,InterpolationLaw='interp1d_linear')
    TwistDict = dict(RelativeSpan=s,Twist=Twist,InterpolationLaw='interp1d_linear')
    if Dihedral:
        DihedralDict = dict(RelativeSpan=s,Dihedral=Dihedral,InterpolationLaw='interp1d_linear')
    else:
        DihedralDict = dict(RelativeSpan=[0,1],Dihedral=[0,0],InterpolationLaw='interp1d_linear')
    if Sweep:
        SweepDict = dict(RelativeSpan=s,Sweep=Sweep,InterpolationLaw='interp1d_linear')
    else:
        SweepDict = dict(RelativeSpan=[0,1],Sweep=[0,0],InterpolationLaw='interp1d_linear')
    ...
    FoilInterpLaw = I.getValue(I.getNodeFromName1(PolarInfoNode, 'InterpolationLaw'))
    ...
    Airfoil = dict(RelativeSpan=Abscissa,Airfoil=AirfoilsGeom,
...
def _computeGradientTensorNodes__(zone,vector=['VelocityX','VelocityY','VelocityZ']):
    '''
    PRIVATE FUNCTION.
    Compute the gradient tensor (djVi) of provided 3-component
    vector and put result on nodes (in FlowSolution).

    INPUTS
    zone - (PyTree Zone) containing requested vector fields.

    OUTPUTS
    None (<t> is modified) - Include new tensor in FlowSolution
    '''

    # Compute gradient in new reference copy of zone
    VarsNames = []
    zoneref = I.copyRef(zone)
    for v in vector:
        zoneref = P.computeGrad(zoneref,v)
        for ic in ('x','y','z'):
            C.center2Node__(zoneref,'centers:grad%s%s'%(ic,v),cellNType=0)
            VarsNames += ['grad%s%s'%(ic,v)]
    VarsArrays = J.getVars(zoneref,VarsNames)

    # Migrate tensor
    for iv in range(len(VarsNames)):
        J._setField(zone,VarsNames[iv],VarsArrays[iv])

    return None

def _computeLiftingLineSources(t,dt):
    '''
    Compute the vorticity sources of the LiftingLines, as
    the equation:
       {gammaSource} = - d{Gamma}/dt + [GradVel].{Gamma}
    ...
    OUTPUTS
    None (in-place function) - Updates gammaSourceX,Y,Z fields
    '''
    for LiftingLine in I.getZones(t):
        ...
        # This function requires the velocity gradient tensor
        _computeGradientTensorNodes__(LiftingLine,vector=['VelocityX','VelocityY','VelocityZ'])

        #              GET ALL REQUIRED POINTERS
        # Circulation bound
        GX,GY,GZ,GXm,GYm,GZm = J.getVars(LiftingLine,['GammaX','GammaY','GammaZ','GammaXm1','GammaYm1','GammaZm1'])
        # Velocity gradient
        dudx,dvdx,dwdx, \
        dudy,dvdy,dwdy, \
        dudz,dvdz,dwdz  = J.getVars(LiftingLine,
        ...
        # source terms
        gSX, gSY, gSZ = J.getVars(LiftingLine,['gammaSourceX','gammaSourceY','gammaSourceZ'])

        # Compute the source of vorticity along LiftingLine
        gSX[:] = -(GX-GXm)/dt + dudx*GX+dvdx*GY+dwdx*GZ
        gSY[:] = -(GY-GYm)/dt + dudy*GX+dvdy*GY+dwdy*GZ
        gSZ[:] = -(GZ-GZm)/dt + dudz*GX+dvdz*GY+dwdz*GZ

        # Migrate values into VPM sources
        VPMinfoDic = J.get(LiftingLine,'.VPM#Info')
        if VPMinfoDic == {}:
            raise AttributeError("No '.VPM#Info' node found in LiftingLine '%s'. Please make sure to use _setVPMinfo2LiftingLine() beforehand."%LiftingLine[0])
        s, = J.getVars(LiftingLine,['s'])
...
def updateLocalFrame(LiftingLine, RotAxis):
    '''
    ...
    RAxyz = np.vstack((RotAxis[0],RotAxis[1],RotAxis[2]))
    ...
    # print('t,b,n')
    # print(txyz[:,0],np.sqrt(txyz[0,0]**2+txyz[1,0]**2+txyz[2,0]**2))
    # print(bxyz[:,0],np.sqrt(bxyz[0,0]**2+bxyz[1,0]**2+bxyz[2,0]**2))
    # print(nxyz[:,0],np.sqrt(nxyz[0,0]**2+nxyz[1,0]**2+nxyz[2,0]**2))
...
def _setMotionConstantRPM(Propeller,RPM):
    '''
    Add the RigidMotion-prescribed motion nodes to a Propeller,
    setting a simple constant RPM motion.

    INPUTS

    Propeller (CGNSBase) - As resulted from buildPropeller()

    RPM - (float) - Used to indicate the propeller's rotational
        speed as revolutions per minute.

    OUTPUTS

        None - Propeller is modified (in-place function)
    '''
    import RigidMotion.PyTree as RM

    Blades = I.getZones(Propeller)
    NBlades = len(Blades)

    # Get Kinematics node (Propeller support information)
    Kin_n = I.getNodeFromName1(Propeller,'.Kinematics')
    ...
    # Use RigidMotion module for applying kinematics
    axis_vct = RotAxis * 1.0 # make copy
    if not direct: axis_vct *=-1
    omega = RPM*(np.pi/30.)

    MotionName = 'ConstantRPM'
    RM._setPrescribedMotion3(Propeller, MotionName, axis_pnt=tuple(RotCenter), axis_vct=tuple(axis_vct), omega=omega)

    # Add RPM information to .Kinematics
    I.createUniqueChild(Kin_n,'RPM','DataArray_t',value=RPM)

    return None

def _moveLiftingLines(t, dt):
    '''
    Workaround for computing the instantaneous velocity of each
    node of a PyTree object <t> at the current instant using a
    time-step precision of <dt>.

    This workaround consists in performing the derivative of the
    coordinates of each zone using three instants,

                    (dt-dt/2, dt, dt+dt/2)

    which requires storing 2 additional sets of coordinates and
    a total of 3 prescribedMotion evaluations. (This is costly)

    The resulting zones are modified, including the SolidVelocity
    vector as fields in FlowSolution (located at nodes).

    INPUTS

    t (PyTree containing zones) - will be typically a Propeller
        object.

    dt (float) - timestep for the precision of the calculation.
        Should be the same as current's computation timestep.

    OUTPUTS
    None (in-place function) - argument <t> is modified
    '''
    import RigidMotion.PyTree as RM
    ...
    for zone in I.getZones(t):
        ...
        # Ignore zones that are not LiftingLines
        isLL = checkComponentKind(zone,kind='LiftingLine')
        if not isLL: continue

        # Evaluate position at requested time+dt/2
        RM._moveZone__(zone,dt+0.5*dt)
        xP, yP, zP = J.getxyz(zone)

        # Evaluate position at requested time-dt/2
        RM._moveZone__(zone,-dt)
        xM, yM, zM = J.getxyz(zone)

        # Put zone at the requested time
        RM._moveZone__(zone,+0.5*dt)
        x0, y0, z0 = J.getxyz(zone)


        # Rearrange the coordinates
        xAll = np.vstack((xM,x0,xP))
        yAll = np.vstack((yM,y0,yP))
        zAll = np.vstack((zM,z0,zP))

        svx, svy, svz = J.getVars(zone,['SolidVelocityX','SolidVelocityY','SolidVelocityZ'])

        # Make the derivative (2nd-order accurate) and store
        svx[:] = np.mean(np.diff(xAll,axis=0),axis=0)/(0.5*dt)
        svy[:] = np.mean(np.diff(yAll,axis=0),axis=0)/(0.5*dt)
        svz[:] = np.mean(np.diff(zAll,axis=0),axis=0)/(0.5*dt)
...
def _computeLocalVelocity(t):
    ...
    for b in I.getBases(t):
        ...
        for LiftingLine in I.getZones(b):
            ...
            iVx, iVy, iVz = J.getVars(LiftingLine,['InducedVelocityX','InducedVelocityY','InducedVelocityZ'])
...
def addPerturbationFields(t, PerturbationFields=None):
    '''
    Sets the existing {InducedVelocityX,Y,Z} fields of Lifting
    ...
    simply sets {InducedVelocityX,Y,Z} to zero.
    ...
    if PerturbationFields:
        ...
        C._initVars(AuxiliarDisc,'InducedVelocityX={MomentumX}')
        C._initVars(AuxiliarDisc,'InducedVelocityY={MomentumY}')
        C._initVars(AuxiliarDisc,'InducedVelocityZ={MomentumZ}')
        ...
        C._initVars(t,'InducedVelocityX',0.)
        C._initVars(t,'InducedVelocityY',0.)
        C._initVars(t,'InducedVelocityZ',0.)
...
def _computeLiftingLine3DLoads(LiftingLine, Density, RotAxis, RPM):
    '''
    Compute instantaneous linear loads along a LiftingLine
    object as well as integral loads
    '''
...
def computeGeneralLoadsOfLiftingLine(LiftingLine, Density, RotationAxis, RPM,
        TorqueOrigin=[0,0,0], NBlades=1.0):
    '''
    Compute instantaneous linear loads along a LiftingLine
    object as well as integral loads
    '''
...
def prepareUnsteadyLiftingLine(t, FreestreamVelocity=np.array([0.,0.,0.]),
    ...
    FreestreamVelocity (3-float numpy vector) - 3 cartesian
    ...
    None - in-place function that modifies t. Modifications:
        -> Add .Conditions node (FreestreamVelocity, Density,
            Temperature)
    ...
    J.set(t,'.Conditions',FreestreamVelocity=FreestreamVelocity, Density=Density, Temperature=Temperature)
...
def convertLiftingLine2PUMABladeDef(LiftingLine,PolarName2FileDict,OutputFile='GeomBlade.py'):
    """
    TODO: TO BE IMPLEMENTED
    TODO: TO BE DOCUMENTED
    """
    ...
    Span,Sweep,Dihedral,Chord, Twist = J.getVars(LiftingLine,['Span','Sweep','Dihedral','Chord','Twist'])
    s = W.gets(LiftingLine)
...
def printCo(message, proc=None, color=None):
    ...
    with open('coprocess.log', 'a') as f:
...
def perturbateLiftingLineUsingPUMA(perturbationField, DIRECTORY_PUMA,
    ...
    Fields2Keep = ['InducedVelocityX', 'InducedVelocityY', 'InducedVelocityZ']
    Fields2Remove = []
    ...
    I._renameNode(tPF, 'InducedVelocityX', 'VelocityX')
    I._renameNode(tPF, 'InducedVelocityY', 'VelocityY')
    I._renameNode(tPF, 'InducedVelocityZ', 'VelocityZ')

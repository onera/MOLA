...
def buildVortexParticleSourcesOnLiftingLine(t, AbscissaSegments,
    ...
    for LiftingLine, AbscissaSegment in zip(LiftingLines, AbscissaSegments):
        ...
        AbscissaInterpolation = AbscissaSegment.copy()
        if GhostParticleAtRoot:
            AbscissaInterpolation = np.append(2.*AbscissaInterpolation[0] - AbscissaInterpolation[1], AbscissaInterpolation)
        if GhostParticleAtTip:
            AbscissaInterpolation = np.append(AbscissaInterpolation , 2*AbscissaInterpolation[-1] - AbscissaInterpolation[-2])
        AbscissaInterpolation = np.array(AbscissaInterpolation, dtype=np.float)
        ...
        if IntegralLaw.startswith('interp1d'):
            ...
            for fieldname in FieldsNames2Extract:
                ...
                sourcefields[fieldname] = interpolator(AbscissaInterpolation)

        elif IntegralLaw == 'linear':
            for fieldname in FieldsNames2Extract:
                sourcefields[fieldname] = np.interp(AbscissaInterpolation,
                                                    v['s'],
                                                    v[fieldname])
                if GhostParticleAtRoot:
            ...
            for fieldname in FieldsNames2Extract:
                ...
                interpolator = si.PchipInterpolator(v['s'], v[fieldname])
                sourcefields[fieldname] = interpolator(AbscissaInterpolation,extrapolate=True)
